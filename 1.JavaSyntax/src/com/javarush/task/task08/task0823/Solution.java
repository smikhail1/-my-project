...
public class Solution {
    public static void main(String[] args) throws IOException {
        ...
        StringBuilder cleaned_str = new StringBuilder(s.trim());

        // String strm = s.trim();
        for (int i=0;i<cleaned_str.length(); i++) {
            if (!cleaned_str.substring(i, i + 1).equals(" ")) {
                current_part.append(cleaned_str.substring(i, i+1));
            }

            if (cleaned_str.substring(i, i + 1).equals(" ") || i == cleaned_str.length()-1) {
        ...
        for (String str: lst) {
            ...
            res.append(str.substring(0, 1).toUpperCase()).append(str.substring(1)).append(" ");
        }
